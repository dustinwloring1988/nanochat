...
class WeightedSourceSampler:
    ...
    def update_weights(self, source_weights: Dict[str, float]):
        ...
        self.weights = torch.tensor([source_weights[name] for name in self.source_names])
...
class DocumentBatchIterator:
    ...
    def __init__(
        ...
        assert len(self.parquet_paths) > 0, \
        ...
        if split == "val":
            ...
            self.parquet_paths = self.parquet_paths[:-1] if len(self.parquet_paths) > 1 else self.parquet_paths
...
def tokenizing_distributed_data_loader_multi_source(
    ...
    sampler = WeightedSourceSampler(source_weights)
    ...
    for source_name in source_weights.keys():
        ...
        if has_subsets and subset_weights and source_name in subset_weights:
            # Multi-subset source: create iterator for each subset
            source_subset_weights = subset_weights[source_name]
            subset_samplers[source_name] = WeightedSourceSampler(source_subset_weights)
            ...
            for subset_name in source_subset_weights.keys():
                ...
                if resume_state_dict and iterator_key in resume_state_dict.get("source_states", {}):
                ...
                doc_iterators[iterator_key] = iter(DocumentBatchIterator(
