...
class CurriculumStage:
    ...
    subset_weights: Dict[str, Dict[str, float]] = field(default_factory=dict)  # Per-source subset weights
    
    def __post_init__(self):
        """Validate stage configuration."""
        assert 0.0 < self.token_ratio <= 1.0, f"token_ratio must be in (0, 1], got {self.token_ratio}"
        assert self.context_range[0] > 0, f"min context must be positive, got {self.context_range[0]}"
        assert self.context_range[0] <= self.context_range[1], \
        ...
        for source, subset_weights in self.subset_weights.items():
            if subset_weights:
                ...
                assert 0.99 <= total_subset_weight <= 1.01, \
...
class CurriculumScheduler:
    ...
    def __init__(
        ...
        # Calculate stage boundaries (cumulative token counts)
        self.stage_boundaries = []
        cumulative_tokens = 0
        for stage in stages:
            stage_tokens = int(stage.token_ratio * total_tokens)
            cumulative_tokens += stage_tokens
            self.stage_boundaries.append(cumulative_tokens)
        
        # Adjust last boundary to exactly match total_tokens (avoid rounding errors)
        self.stage_boundaries[-1] = total_tokens
        
        # Calculate stage boundaries in steps if total_steps provided
        if total_steps is not None:
            self.stage_step_boundaries = []
            cumulative_steps = 0
            for stage in stages:
                stage_steps = int(stage.token_ratio * total_steps)
                cumulative_steps += stage_steps
                self.stage_step_boundaries.append(cumulative_steps)
    ...
    def get_stage_progress(
        ...
        if tokens_processed is not None:
            ...
            stage_start = 0 if stage_idx == 0 else self.stage_step_boundaries[stage_idx - 1]
    ...
    def get_stage_info(
        ...
        stage_start_tokens = 0 if stage_idx == 0 else self.stage_boundaries[stage_idx - 1]
        ...
        if self.stage_step_boundaries is not None:
            stage_start_steps = 0 if stage_idx == 0 else self.stage_step_boundaries[stage_idx - 1]
    ...
    def print_curriculum_plan(self):
        ...
        for idx, stage in enumerate(self.stages):
            ...
            if self.stage_step_boundaries:
                stage_start_step = 0 if idx == 0 else self.stage_step_boundaries[idx - 1]
