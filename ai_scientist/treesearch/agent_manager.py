    ...
    def __init__(self, task_desc: str, cfg: Any, workspace_dir: Path):
        ...
        if self.cfg.experiment.mode == "nanochat":
            self.main_stage_goals = {
                1: "Reproduce the fixed-token nanochat baseline and canonical results contract.",
                2: "Tune nanochat optimizer, schedule, batch, and curriculum data settings without changing model architecture.",
                3: "Explore principled nanochat architecture or training-method changes within the fixed VRAM and token budget.",
                4: "Ablate the strongest stage-three change using the same data, seed, and evaluation contract.",
            }
...
"""
    ...
    def _check_substage_completion(
        ...
        if self.cfg.experiment.mode == "nanochat":
            if len(journal.nodes) >= current_substage.max_iterations:
                return True, "Nanochat node budget reached"
            return False, "Nanochat canonical candidate budget remains"
    ...
    def _check_stage_completion(self, stage: Stage) -> bool:
        ...
        if self.cfg.experiment.mode == "nanochat":
            best_node = journal.get_best_node(cfg=self.cfg)
            if not best_node:
                return False, "No valid nanochat candidate found"
            if stage.stage_number > 1 and best_node == journal.nodes[0]:
                return False, "Nanochat candidate did not improve the inherited node"
            return True, "Nanochat canonical candidate accepted"
    ...
    def _create_next_substage(
        ...
        sub_stage_name = re.sub(r"[^A-Za-z0-9_-]+", "_", sub_stage_name).strip("_")
        if not sub_stage_name:
            sub_stage_name = "unnamed"
    ...
    def _node_limit_reached(self) -> bool:
        max_nodes = self.cfg.agent.max_nodes
        return (
            max_nodes is not None and max_nodes > 0 and self.executed_nodes >= max_nodes
        )
                ...
                with self._create_agent_for_stage(current_substage) as agent:
                    ...
                    while True:
                        if self._node_limit_reached():
                            ...
                            current_substage = None
                            break
                        processed_nodes = agent.step(exec_callback)
                        self.executed_nodes += processed_nodes or 0
                        if step_callback:
                            step_callback(
                        ...
                        if self._node_limit_reached():
                            logger.info(
                                f"Stopping after {self.executed_nodes} executed nodes"
                            )
                            self.current_stage = None
                            current_substage = None
                            break
