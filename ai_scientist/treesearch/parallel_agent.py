...
_ACTIVE_INTERPRETERS: Set[object] = set()


def _worker_cleanup_handler(signum, _frame):
    for interpreter in list(_ACTIVE_INTERPRETERS):
        try:
            interpreter.cleanup_session()
        except Exception:
            logger.exception("Failed to clean worker interpreter")
    os._exit(128 + signum)


def _install_worker_cleanup_handler():
    try:
...
def _register_worker_interpreter(interpreter):
    _ACTIVE_INTERPRETERS.add(interpreter)


def _unregister_worker_interpreter(interpreter):
    _ACTIVE_INTERPRETERS.discard(interpreter)
...
class MinimalAgent:
    ...
    def _prompt_environment(self):
        if self.cfg.experiment.mode == "nanochat":
            return {
                "Nanochat environment": "Use only the copied nanochat source tree, PyTorch, and packages already installed in the AI Scientist runtime."
            }
        pkgs = [
            "numpy",
            "pandas",
    ...
    def _prompt_impl_guideline(self):
        if self.cfg.experiment.mode == "nanochat":
            return {
                "Nanochat implementation guideline": [
                    "The candidate source tree is NANOCHAT_SOURCE_DIR and the only writable output directory is NANOCHAT_WORKING_DIR.",
                    "Do not modify the repository root, shared cache, provider credentials, or files outside the candidate source and working directories.",
                    "Preserve the fixed-token experiment contract, the declared seed, and the 16 GB VRAM limit.",
                    "The primary metric is validation bits per byte, lower is better. Runtime, throughput, CORE, and VRAM are guardrails.",
                    "The program must finish by calling nanochat.ai_scientist_experiment.run_from_environment with the working directory.",
                    "run_from_environment must create results.json and deterministic PNG plots in the working directory.",
                    "Do not create experiment_data.npy, parse metrics with another model, or implement a second training loop.",
                    "Return one self-contained Python code block and execute immediately without a __main__ guard.",
                ]
            }
        impl_guideline = [
        ...
        if self.cfg.experiment.mode == "nanochat":
            prompt["Instructions"] |= {
                "Experiment design sketch guideline": [
                    "Reproduce the supplied nanochat runfile and canonical experiment configuration.",
                    "Do not introduce new datasets or change the tokenizer, held-out split, primary metric, or result schema.",
                    "Keep the initial implementation simple and compatible with one RTX 4060 Ti.",
                ]
            }
        else:
            prompt["Instructions"] |= {
...
class ParallelAgent:
    def __init__(
        ...
        if self.num_gpus == 0:
            if cfg.experiment.mode == "nanochat":
                raise RuntimeError("nanochat experiments require a CUDA GPU")
        ...
        self.timeout = self.cfg.exec.timeout + 90
        self.executor = ProcessPoolExecutor(max_workers=self.num_workers)
        self._controller_attestation_key = secrets.token_bytes(32)
    ...
    def _define_global_metrics(self) -> str:
        ...
        if self.cfg.experiment.mode == "nanochat":
            return "val_bpb (lower is better)"
        prompt = {
            "Introduction": (
    ...
    def _run_multi_seed_evaluation(self, node: Node) -> List[Node]:
        ...
        for future in futures:
            try:
                ...
                if "__worker_error__" in result_data:
                    raise RuntimeError(result_data["__worker_error__"])
                ...
                future.cancel()
                self._abort_executor()
                raise RuntimeError("Multi-seed worker timed out") from exc
            except Exception as e:
    ...
    def _process_node_wrapper(
        ...
        _install_worker_cleanup_handler()
        ...
        if cfg.experiment.mode == "nanochat":
            paths = prepare_node_workspace(
                cfg, parent_node.candidate_source if parent_node else None
            )
            workspace = str(paths["workspace"])
            working_dir = str(paths["working"])
            environment = experiment_env(cfg, paths, gpu_id)
        else:
            task_id = uuid.uuid4().hex
            workspace = os.path.join(cfg.workspace_dir, "nodes", task_id)
        ...
        if cfg.experiment.mode == "nanochat" and gpu_id is None:
            raise RuntimeError("nanochat experiment was not assigned a GPU")
        ...
        interpreter_working_dir = workspace
        process_interpreter = Interpreter(
            working_dir=interpreter_working_dir,
            timeout=cfg.exec.timeout,
            format_tb_ipython=cfg.exec.format_tb_ipython,
            agent_file_name=cfg.exec.agent_file_name,
            env_vars=environment,
            sanitize_environment=cfg.exec.sanitize_environment,
        )
        _register_worker_interpreter(process_interpreter)
        ...
        try:
            ...
            if cfg.experiment.mode == "nanochat":
                apply_nanochat_result(child_node, exec_result, cfg, paths)
                result_data = child_node.to_dict()
                print("Returning canonical nanochat result")
                return result_data
            ...
            traceback.print_exc()
            return {"__worker_error__": f"{type(e).__name__}: {str(e)[:2000]}"}
        finally:
            _unregister_worker_interpreter(process_interpreter)
            process_interpreter.cleanup_session()
    ...
    def step(self, exec_callback: ExecCallbackType):
        ...
        if self.cfg.agent.get("summary", None) is not None:
            memory_summary = self.journal.generate_summary(
                ...
                **{
                    ...
                    "max_tokens": self.cfg.agent.summary.max_tokens,
                },
            )
        else:
        ...
        for i, future in enumerate(futures):
            try:
                ...
                result_node = Node.from_dict(result_data, self.journal)
                if self.cfg.experiment.mode == "nanochat" and not result_node.is_buggy:
                    attested_payload = attest_nanochat_artifact(
                        Path(result_node.exp_results_dir),
                        self.cfg,
                        self._controller_attestation_key,
                        result_node.id,
                        Path(result_node.exp_results_dir).name,
                    )
                    if (
                        result_node.metric.get_mean_value()
                        != attested_payload["primary_metric"]["value"]
                    ):
                        raise ValueError("Node metric does not match attested result")
