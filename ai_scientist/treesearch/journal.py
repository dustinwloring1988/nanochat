...
def _serializable_path(path: str | os.PathLike) -> str:
...
class Node(DataClassJsonMixin):
    ...
    candidate_source: str = field(default=None, kw_only=True)  # type: ignore
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "candidate_source": (
                _serializable_path(self.candidate_source)
            ...
            "plots": self.plots,
            "plot_paths": (
                [_serializable_path(path) for path in self.plot_paths]
                if self.plot_paths
                else []
            ),
            "plot_analyses": [
...
class Journal:
    ...
    def get_best_node(
        ...
        if cfg is not None and cfg.agent.metric_only:
        ...
        try:
            if cfg is None or cfg.agent.get("select_node", None) is None:
                raise ValueError("select_node model is required")
            model = cfg.agent.select_node.model
            temperature = cfg.agent.select_node.temp
            max_tokens = cfg.agent.select_node.max_tokens
            selection = query(
