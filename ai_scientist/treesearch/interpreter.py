...
def _environment_allowed(name: str) -> bool:
    return name in ALLOWED_ENV_NAMES or name.startswith(ALLOWED_ENV_PREFIXES)


def _signal_process_tree(process: Process, sig: int) -> None:
...
class Interpreter:
    ...
    def child_proc_setup(self, result_outq: Queue) -> None:
        ...
        if self.sanitize_environment:
            for key in list(os.environ):
                if not _environment_allowed(key):
                    os.environ.pop(key, None)

        for key, value in self.env_vars.items():
            os.environ[key] = value

        source_dir = os.environ.get("NANOCHAT_SOURCE_DIR")
        root_paths = {
            os.getcwd(),
            os.environ.get("NANOCHAT_SOURCE_ROOT", ""),
        }
        sys.path[:] = [path for path in sys.path if path not in root_paths]
        if source_dir:
    ...
    def cleanup_session(self):
        if self.process is None:
            return
        _signal_process_tree(self.process, signal.SIGTERM)
        self._drain_queues()
        self.process.join(timeout=2)
        if self.process.exitcode is None:
            logger.warning("Child process failed to terminate gracefully, killing it..")
            _signal_process_tree(self.process, signal.SIGKILL)
            self._drain_queues()
            self.process.join(timeout=2)
        ...
        while True:
            try:
                ...
                if running_time > self.timeout:
                    ...
                    _signal_process_tree(self.process, signal.SIGINT)
