...
def prep_cfg(cfg: Config):
    ...
    if cfg.agent.max_nodes is not None and cfg.agent.max_nodes < 0:
        raise ValueError("agent.max_nodes must be non-negative; zero means unlimited")
    cfg.experiment.source_dir = Path(cfg.experiment.source_dir).resolve()
    ...
    if cfg.experiment.mode == "nanochat":
        ...
        if not cfg.agent.metric_only:
            raise ValueError("nanochat experiments require metric-only selection")
        if cfg.agent.multi_seed_eval.num_seeds != 0:
            raise ValueError("nanochat experiments require multi_seed_eval.num_seeds=0")
        if cfg.agent.max_nodes != 1:
            raise ValueError("nanochat experiments require max_nodes=1")
