"""
Train model with multi-stage curriculum learning.

This extends base_train.py with curriculum support:
- Multi-source data mixing with configurable weights per stage
- Context length scheduling (gradual ramps within stages)
- Stage transitions with logging
- Full resumption support including curriculum state
...
For simple testing without curriculum (falls back to original behavior):
    python -m scripts.base_train_curriculum --depth=6 --num-iterations=100
"""
...
use_curriculum = args.config is not None or args.curriculum_mode
...
output_dirname = args.model_tag if args.model_tag else f"d{args.depth}_curriculum"
...
# -----------------------------------------------------------------------------
# Scaling laws and training horizon
...
if use_curriculum:
    # Calculate num_iterations first
    ...
    args.max_seq_len = initial_context
else:
