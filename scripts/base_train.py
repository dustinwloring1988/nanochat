...
def make_resume_contract():
    return build_resume_contract(
        model_config=model_config_kwargs,
        seed=args.seed,
        world_size=ddp_world_size,
        rank=ddp_rank,
        token_horizon=total_tokens,
...
if resuming:
    ...
    validate_resume_contract(meta_data["resume_contract"], make_resume_contract())
...
while True:
    ...
    train_loss_f = (
        torch.stack(micro_losses).mean().item()
    )  # .item() is a CPU-GPU sync point
    synchronize()
    t1 = time.time()
    dt = t1 - t0
    # -------------------------------------------------------------------------
    ...
    print0(
        f"step {step:05d}/{num_iterations:05d} ({pct_done:.2f}%) | loss: {debiased_smooth_loss:.6f} | lrm: {lrm:.2f} | dt: {dt * 1000:.2f}ms | tok/sec: {tok_per_sec:,} | bf16_mfu: {mfu_display} | epoch: {epoch} | total time: {total_training_time/60:.2f}m{eta_str}"
    )
    if step % 100 == 0:
        log_data = {
            "step": step,
