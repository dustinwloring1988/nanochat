...
# Extended speed run configured for a single NVIDIA RTX 4060 Ti 16GB GPU.
# Running for TWICE as long as the original speedrun-4060ti.sh (200 iterations vs 100).
# Expected runtime: ~30-40 minutes depending on model size.
...
if [ ! -d "$NANOCHAT_BASE_DIR/base_checkpoints" ]; then
    ...
    # *** DOUBLED iterations from 100 to 200 for 2x training duration ***
